...
@Tag(name = "user-area" , description = "The user area, where the own user performs operations")  // Como será chamado o nome do "controller" no swagger e a descrição.
public interface UserAreaController { // Controller de UserArea deve implementar essa interface

    @Operation(summary = "Access the logged user informations", security = {@SecurityRequirement(name = "bearer-key")}) // SecurityRequirement - Segurança de metodo no Swagger
    ...
    @Operation(summary = "Change the account password", security = {@SecurityRequirement(name = "bearer-key")}) // SecurityRequirement - Segurança de metodo no Swagger
    @ApiResponses(value = {                 // Informações relacionadas ao response!
